...
public class RelicManager : MonoBehaviour
{
    ...
    {
        int levelNumber = tower != null ? tower.CurrentLevelNumber : 1;
        TowerData towerData = tower != null ? tower.data : null;
        return ApplyDamageForLevel(towerData, levelNumber, baseDamage);
    }

    /// <summary>
    /// Canonical stat-preview path. UI and live towers both use this so tower-specific relics
    /// (especially Cannon Hero's +damage per level) cannot disagree with gameplay.
    /// </summary>
    public float ApplyDamageForLevel(TowerData towerData, int levelNumber, float baseDamage)
    {
        float damage = ApplyDamage(baseDamage) + GetDamageFromLives();
        if (IsCannonHeroTower(towerData))
            damage += cannonHeroDamagePerLevel * Mathf.Max(1, levelNumber);
        return damage;
    }
    ...
    {
        return ApplyRange(tower != null ? tower.data : null, baseRange);
    }

    /// <summary>Data-based overload used by ghost/build previews where no Tower instance exists yet.</summary>
    public float ApplyRange(TowerData towerData, float baseRange)
    {
        float result = ApplyRange(baseRange);
        if (IsCannonHeroTower(towerData)) result += cannonHeroRangeFlat;
        return result;
    }

    public bool IsCannonHeroTower(TowerData towerData)
    {
        return cannonHeroActive && cannonHeroTower != null && towerData == cannonHeroTower;
    }

    public float GetMaxProjectileDamage(TowerData towerData, float launchDamage)
    {
        if (!IsCannonHeroTower(towerData)) return launchDamage;
        return launchDamage * (1f + Mathf.Max(0f, cannonHeroTravelBonusCap));
    }
    ...
    {
        if (!IsCannonHeroTower(sourceTower)) return damage;
    ...
    {
        if (!IsCannonHeroTower(towerData)) return true;
        return !cannonHeroPurchaseUsed;
    }

    public void NotifyTowerBuilt(TowerData towerData)
    {
        if (IsCannonHeroTower(towerData)) cannonHeroPurchaseUsed = true;
    }
