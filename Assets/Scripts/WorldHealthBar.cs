...
public class WorldHealthBar : MonoBehaviour
{
    ...
    public void SetData(Enemy e)
    {
        ...
        if (bar != null)
        {
            bar.Initialize(e.MaxHP);
            bar.gameObject.SetActive(false);
        }

        if (shieldBar != null)
        {
            shieldBar.Initialize(e.MaxHP);
    ...
    public void Refresh(bool isHeal = false)
    {
        ...
        bool shouldShow = enemy.IsAlive && hp < enemy.MaxHP - ValueEpsilon;
        ...
        if (!valueChanged && !animationDirectionChanged) return;
    ...
    public void RefreshShield(bool isGain = false)
    {
        ...
        if (!shown)
        {
            lastShield = 0f;
            lastShieldWasGain = false;
            if (shieldBar.gameObject.activeSelf) shieldBar.gameObject.SetActive(false);
            return;
        }

        if (!shieldBar.gameObject.activeSelf) shieldBar.gameObject.SetActive(true);
    ...
    private void LateUpdate()
    {
        ...
        if (modelRenderer != null) pos.y = modelRenderer.bounds.max.y;
        else pos.y += fallbackOffsetY;
